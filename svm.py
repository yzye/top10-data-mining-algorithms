...
def svm_train(instances, labels, kernel_func, C=1.0):
    ...
    def func(alpha):
        ...
        return -np.sum(alpha) + (1.0/2)*alpha.dot(kernel_mat* pm_labels[np.newaxis].T.dot(pm_labels[np.newaxis])).dot(alpha)
    ...
    def func_deriv(alpha):
        ...
        return -np.ones(n) + (kernel_mat* pm_labels[np.newaxis].T.dot(pm_labels[np.newaxis])).dot(alpha)
    ...
    alpha_y_nz = np.array(np.nonzero(alpha_y))[0]#np.extract(alpha_y, alpha_y)

    # TASK 2.6
    # retain those instances with non-zero alpha_y entries
    # these are the "support vectors"
    support_vectors = instances[alpha_y_nz]#instances[[index for index,value in enumerate(alpha_y) if value !=0 ],]
    ...
    def classifier(point):
        ...
        return 1 if sum([alpha_y[i]*kernel_func(point,instances[i]) for i in alpha_y_nz]) > 0 else 0
...
def evaluate_classifier(classifier, instances, labels):
    ...
    positives = instances[[i for i in range(len(labels)) if labels[i]>0]]
    ...
    pos_predictions = [classifier(instances[j]) for j in pos_labels]
    pos_correct = sum(pos_predictions)

    # TASK 2.8.3
    # extract negative instances, their labels
    negatives = instances[[i for i in range(len(labels)) if labels[i]<=0]]
    neg_labels = np.where(labels==0)[0]

    # TASK 2.8.4
    # find the predictions of classifier on negatives
    # and count the no. of correct predictions therein
    neg_predictions = np.array([classifier(instances[j]) for j in neg_labels])
