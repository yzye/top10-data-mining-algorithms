...
def kmeans_plus_plus(instances, K):
    ...
    m,n = len(instances),len(instances[0])
    centers = [[0 for a in range(n)] for b in range(K)]
    index = random.randint(1,m)
    centers[0] = instances[index] 
    for i in range(1, K): 
        distances = [[0 for c in range(i)] for d in range(m)]
        for j in range(m):
            for l in range(i):
                distances[j][l] = euclidean_squared(instances[j], centers[l])
        nearestDistances = [min(dt) for dt in distances]
        sumDis = sum(nearestDistances) 
        randomNum = random.uniform(0,1)  * sumDis
        for p in range(m):
            randomNum = randomNum - nearestDistances[p]
            if randomNum <= 0:
                centers[i] = instances[p]
                break
    return centers
...
def recompute_centers(instances, cluster_ids, centers):
    ...
    for i in range(K):
        ...
        one_cluster = [j for j,eval in enumerate(cluster_ids) if eval==i]
        ...
        sum_cluster = reduce(lambda x, y: [m+n for m,n in zip(x,y)], [instances[m] for m in one_cluster ])
