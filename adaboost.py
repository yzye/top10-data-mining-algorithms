...
def weak_learner(instances, labels, dist):
    ...
    """ Returns the best 1-d threshold classifier.s
    ...
    """
    error={np.sum(np.abs(1*(sign*instances.T[col_num]<sign*threshold)-labels)*dist):(col_num,threshold,sign) \
           for col_num in range(instances.shape[1]) \
           for threshold in np.unique(np.concatenate([[-1],instances.T[col_num],[1]])) \
           for sign in [-1,1]}
    
    col_num,threshold,sign = error[np.min(list(error.keys()))]
    
    return lambda x: 1*(sign*x.T[col_num]<sign*threshold)
...
def compute_error(h, instances, labels, dist):
    ...
    """
    errArr = np.ones(len(labels))
    errArr [labels==h(instances)] =0
    return  sum(dist*errArr)
...
def update_dist(h, instances, labels, dist, alpha):
    ...
    errArr = np.ones(len(labels))
    errArr [labels==h(instances)] = -1
    new_dist = dist*map(math.exp,alpha*errArr)
...
def run_adaboost(instances, labels, weak_learner, num_iters=20):
    ...
    def classifier(point):
        ...
        return 1 if sum(list(map(lambda y: y[0]*(2*(y[1](point))-1), alpha_h))) > 0 else 0
