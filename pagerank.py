...
def main():
    ...
    adj_mat_norm = np.nan_to_num(adj_mat / out_links[:, np.newaxis])
    # TASK 1.4
    # make matrix stochastic
    rank_one_change = np.matrix(dangling).T.dot(np.ones((1,n)))/n
    ...
    top_10 = np.argsort(pageranks)[0,-10:][0,::-1]
    ...
    close_to_1 = np.array(np.ndarray.nonzero(np.isclose(np.ones(n),w))[0])
    ...
    #list= [np.count_nonzero(edges[:,1]==i) for i in top_10]
